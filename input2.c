...
void get_commands(Command **commands, int *command_count, ListNode **head);
void process_commands(Command *commands, int command_count, ListNode **head);
int is_list_empty(ListNode *head);
...
void get_commands(Command **commands, int *command_count, ListNode **head) {
    ...
    *command_count = 0; // Initialize command count to 0
    ...
    while (!is_list_empty(*head)) { // Loop until the linked list becomes empty
        ...
        // For "close" and "switch" commands, also read the number
        if (strcmp(cmd->str, "close") == 0 || strcmp(cmd->str, "switch") == 0) {
        ...
        // Process the command immediately and update the linked list
        process_commands(*commands + *command_count - 1, 1, head);
        printList(*head); // Print the linked list after each command
    }

    printf("The linked list is empty. Exiting the program.\n");
}
    ...
    for (int i = 0; i < command_count; i++) {
        if (strcmp(commands[i].str, "open") == 0) {
            ...
            *head = deleteNode(*head, commands[i].number);
            ...
            // Implement a specific action for "switch" if needed
...
int main() {
    ...
    // Get command input from user and process commands immediately
    get_commands(&commands, &command_count, &myList);
